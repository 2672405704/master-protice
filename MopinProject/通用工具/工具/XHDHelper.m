...
{
    NSString *resultStr;
    if(markStr.length>length)
    {
        resultStr = [NSString stringWithFormat:@"%@...%@",[markStr substringToIndex:range.location],[markStr substringFromIndex:markStr.length-range.location]];
...
+(UIButton*)getRightArrowButtonWith:(CGRect)frame
{
    ...
    [right setImage:[UIImage imageNamed:@"chevron_right"] forState:UIControlStateNormal];
    return right;
}
...
+ (UIButton*)getRedGeneralButtonWithFrame:(CGRect)frame  AndTitleString:(NSString*)tittle
{
    ...
    bnt.frame = frame;
    [bnt setTitle:tittle forState:UIControlStateNormal];
    [bnt setTitle:tittle forState:UIControlStateSelected];
    bnt.titleLabel.font = UIFONT_Tilte(16);
    [bnt setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
    bnt.backgroundColor = THEMECOLOR_1;
    return bnt;

}

/*TODO:创建一个人正常为白色，点击为红色，导角为5，边线宽为0.8的按钮*/
+ (UIButton*)getRedGeneralChooseButtonWithFrame:(CGRect)frame
                AndTitleString:(NSString*)tittle
{
    UIButton *bnt = [UIButton buttonWithType:UIButtonTypeCustom];
    ...
    [bnt setTitleColor:MainFontColor forState:UIControlStateNormal];
    [bnt setTitleColor:[UIColor whiteColor] forState:UIControlStateSelected];
    
    [bnt setBackgroundImage:mImageByName(@"white_bg_01") forState:UIControlStateNormal];
      [bnt setBackgroundImage:mImageByName(@"red_bg") forState:UIControlStateSelected];
    [bnt setTitleEdgeInsets:UIEdgeInsetsMake(0, 10, 0, 0)];
    
//    bnt.layer.borderColor = bnt.isSelected?[UIColor clearColor].CGColor:MainFontColor.CGColor;
//    bnt.layer.borderWidth = 0.5;
//    bnt.layer.cornerRadius = 4;
                ...
                selector:(SEL)selector
{
    UIButton *bnt = [UIButton buttonWithType:UIButtonTypeCustom];
    bnt.frame = frame;
    [bnt setTitle:nomalTitle forState:UIControlStateNormal];
    [bnt setTitle:selectedTitle forState:UIControlStateSelected];
    
    [bnt setTitleColor:nomTitColor forState:UIControlStateNormal];
    [bnt setTitleColor:selTitColor forState:UIControlStateSelected];
    
    [bnt setImage:nomalImage forState:UIControlStateNormal];
    [bnt setImage:selectedImage forState:UIControlStateSelected];
    
    bnt.layer.borderColor = nomTitColor.CGColor;
    bnt.layer.borderWidth = width;
    bnt.layer.cornerRadius = 4;
    bnt.clipsToBounds = YES;
...
#pragma mark -- 创建一个输入框工具条
+(void)addToolBarOnInputFiled:(id)txField Action:(SEL)hideKeyBoard Target:(id)target
{
    //TODO:工具条，导航控制器右键控制
    UIToolbar *toolbar = [[UIToolbar alloc] initWithFrame:CGRectMake(0, -36, kkDeviceWidth, 36)];
    ...
    [doneBnt setTitleColor:THEMECOLOR_1 forState:UIControlStateNormal];
...
+ (void)addDivLineWithFrame:(CGRect)frame SuperView:(UIView*)fatherView
{
    ...
    view.backgroundColor = DIVLINECOLOR_1;
...
#pragma mark --  获取今天日期的字符串YY-MM-DD
+(NSString*)getToday
{
    //获取今天的日期
